...
class CreateWebhookTaskSpec extends Specification {
  ...
  def "should create new webhook task with expected parameters"() {
    setup:
    def stage = new StageExecution(pipeline, "webhook", "My webhook", [
  ...
  def "should default to POST if no method is specified"() {
    setup:
    def stage = new StageExecution(pipeline, "webhook", [
      url: "https://my-service.io/api/"
    ])
  ...
  def "should return TERMINAL on URL validation failure"() {
    setup:
    def stage = new StageExecution(pipeline, "webhook", "My webhook", [url: "wrong://my-service.io/api/"])
  ...
  def 'should honor the web hook #webHookUrl protocol/scheme for status check URL #responseStatusCheckUrl'() {
    ...
    def stage = new StageExecution(pipeline, "webhook", "Protocol scheme webhook", stageContext)
