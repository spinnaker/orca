...
class GetCommitsTask implements DiffTask {
  ...
  TaskResult execute(Stage stage) {
    ...
    try {
      ...
      if (!targetAmi) {
        if (stage.execution.trigger.payload instanceof PipelineTriggerPayload) {
          def parentPipeline = stage.execution.trigger?.payload?.parentExecution
          while (!targetAmi && parentPipeline?.context) {
            targetAmi = getTargetAmi(parentPipeline.context, region)
            parentPipeline = parentPipeline.trigger?.payload?.parentExecution
          }
        }
      }
