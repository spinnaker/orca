...
package com.amazon.spinnaker.orca.aws.lambda.eventconfig;
...
import com.amazon.spinnaker.orca.aws.lambda.eventconfig.model.LambdaDeleteEventTaskInput;
import com.amazon.spinnaker.orca.aws.lambda.eventconfig.model.LambdaEventConfigurationDescription;
import com.amazon.spinnaker.orca.aws.lambda.eventconfig.model.LambdaUpdateEventConfigurationTaskInput;
import com.amazon.spinnaker.orca.aws.lambda.eventconfig.model.LambdaUpdateEventConfigurationTaskOutput;
import com.amazon.spinnaker.orca.aws.lambda.model.EventSourceMappingConfiguration;
...
public class LambdaUpdateEventConfigurationTask implements LambdaStageBaseTask {
  ...
  public TaskResult execute(@NotNull StageExecution stage) {
    ...
    if (lf == null) {
      return formErrorTaskResult(
          stage, String.format("Could not find lambda to update event config for"));
    }
  ...
  private void deleteAllExistingEvents(
    ...
    eventArnList.stream()
        .forEach(
            eventArn -> {
              deleteEvent(eventArn, taskInput, lf, targetArn);
            });
  }
  ...
  private LambdaUpdateEventConfigurationTaskOutput deleteRemovedAndChangedEvents(
    ...
    eventArnList.stream()
        .filter(
            x -> {
              return !taskInput.getTriggerArns().contains(x);
            })
        .forEach(
            eventArn -> {
              deleteEvent(eventArn, taskInput, lf, targetArn);
  ...
  List<String> getExistingEvents(LambdaDefinition lf, String targetArn) {
    ...
    List<String> allEventArns =
        esmList.stream()
            .filter(
                y -> {
                  return y.getFunctionArn().equals(targetArn);
                })
            .map(
                x -> {
            ...
            .filter(
                x -> {
                  return StringUtils.isNotNullOrEmpty(x);
                })
  ...
  List<Pair<String, Integer>> getExistingEventsDetails(LambdaDefinition lf) {
    ...
    List<Pair<String, Integer>> allEventArns =
        esmList.stream()
            .filter(
                x -> {
                  return StringUtils.isNotNullOrEmpty(x.getEventSourceArn());
                })
            .map(
                x -> {
                  return Pair.of(x.getEventSourceArn(), x.getBatchSize());
                })
            .collect(Collectors.toList());
    return allEventArns;
  }
  ...
  private LambdaUpdateEventConfigurationTaskOutput updateEventConfiguration(
    ...
    taskInput.getTriggerArns().stream()
        .filter(
            curr -> {
              return !existingEvents.contains(curr);
            })
        .forEach(
            curr -> {
