...
public class BakeCloudFoundryManifestTaskTest {
    ...
    private BakeCloudFoundryManifestTask bakeCloudFoundryManifestTask = new BakeCloudFoundryManifestTask(artifactUtils, Optional.of(bakery));


    @Test
    public void shouldMapStageToContext() throws JsonProcessingException {
        String stageJson = "{\"expectedArtifacts\":" +
    ...
    public void shouldMapContextToBakeCFRequest() throws JsonProcessingException {
        ...
        List<Artifact> resolvedInputArtifacts = context.getInputArtifacts().stream()
                .map(i -> i.getArtifact())
                .collect(Collectors.toList());

        BakeCloudFoundryManifestRequest request = new BakeCloudFoundryManifestRequest(context, resolvedInputArtifacts.get(0), resolvedInputArtifacts.subList(1, resolvedInputArtifacts.size()), context.getOutputName());
    ...
    public void shouldThrowExceptionForEmptyInputArtifacts() throws JsonProcessingException {
        ...
        assertThat(exception.getMessage()).isEqualTo("There must be one manifest template and at least one variables artifact supplied");
    ...
    public void shouldThrowExceptionForEmptyProducedArtifact() throws JsonProcessingException {
        ...
        stage.setContext(mapper.readValue(stageJson, Map.class));
        BakeCloudFoundryManifestContext context = stage.mapTo(BakeCloudFoundryManifestContext.class);
        when(artifactUtils.getBoundArtifactForStage(any(), any(), any())).thenReturn(Artifact.builder().build()).thenReturn(Artifact.builder().build());
        Exception exception = assertThrows(
                IllegalArgumentException.class,
                () -> bakeCloudFoundryManifestTask.execute(stage),
                "Expected it to throw an error but it didn't"
        );
        assertThat(exception.getMessage()).isEqualTo("The CreateCloudFoundryManifest stage produces one embedded base64 artifact.  Please ensure that your stage config's `Produces Artifacts` section (`expectedArtifacts` field) contains exactly one artifact.");
    ...
    public void shouldThrowExceptionForNameMismatch() throws JsonProcessingException {
        ...
        assertThat(exception.getMessage()).isEqualTo("The name of the output manifest is required and it must match the artifact name in the Produces Artifact section.");
