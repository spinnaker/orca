...
class CreateDeployTaskSpec extends Specification {
  ...
  def "prefers the ami from an upstream stage to one from deployment details"() {
    ...
    and:
    def bakeStage1 = new StageExecution(stage.execution, "bake")
    bakeStage1.id = UUID.randomUUID()
    bakeStage1.refId = "1a"
    stage.execution.stages << bakeStage1

    def bakeSynthetic1 =
      new StageExecution(stage.execution, "bake in $deployRegion", [ami: amiName, region: deployRegion, cloudProvider: "aws"])
    bakeSynthetic1.id = UUID.randomUUID()
    bakeSynthetic1.parentStageId = bakeStage1.id
    stage.execution.stages << bakeSynthetic1

    def bakeStage2 = new StageExecution(stage.execution, "bake")
    bakeStage2.id = UUID.randomUUID()
    bakeStage2.refId = "2a"
    stage.execution.stages << bakeStage2

    def bakeSynthetic2 =
      new StageExecution(stage.execution, "bake in $deployRegion", [ami: "parallel-branch-ami", region: deployRegion, cloudProvider: "aws"])
    bakeSynthetic2.id = UUID.randomUUID()
    bakeSynthetic2.parentStageId = bakeStage2.id
    stage.execution.stages << bakeSynthetic2

    def intermediateStage = new StageExecution(stage.execution, "whatever")
  ...
  def "finds the image from an upstream stage matching the cloud provider"() {
    ...
    def bakeSynthetic1 = new StageExecution(stage.execution, "bake in $deployRegion", [ami: amiName, region: deployRegion, cloudProvider: "aws"])
    bakeSynthetic1.id = UUID.randomUUID()
    bakeSynthetic1.parentStageId = bakeStage1.id
    stage.execution.stages << bakeSynthetic1
    ...
    def bakeSynthetic2 = new StageExecution(stage.execution, "bake in $deployRegion", [ami: "different-image", region: deployRegion, cloudProvider: "gce"])
    bakeSynthetic2.id = UUID.randomUUID()
    bakeSynthetic2.parentStageId = bakeStage2.id
    stage.execution.stages << bakeSynthetic2
  ...
  def "find the image from stage matching the cloud provider"() {
    ...
    and:
    def findImage1 = new StageExecution(stage.execution, "findImage")
    findImage1.id = UUID.randomUUID()
    findImage1.refId = "1a"
    stage.execution.stages << findImage1

    def findImageSynthetic1 = new StageExecution(stage.execution, "findImage", [
    ...
    def findImage2 = new StageExecution(stage.execution, "findImage")
    findImage2.id = UUID.randomUUID()
    findImage2.refId = "2a"
    stage.execution.stages << findImage2


    def findImageSynthetic2 = new StageExecution(stage.execution, "findImage", [
      ami: "different-image",
      region: deployRegion,
      cloudProvider: "aws",
      selectionStrategy:"LARGEST",
