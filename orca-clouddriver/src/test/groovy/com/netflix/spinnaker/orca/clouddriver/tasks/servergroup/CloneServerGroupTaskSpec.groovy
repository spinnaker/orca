...
class CloneServerGroupTaskSpec extends Specification {
  ...
  def "amiName uses value from bake"() {
    given:
    def bakeEast = new StageExecution(stage.execution, "bake", [ami: bakeAmi, region: 'us-east-1', cloudProvider: 'aws'])
    bakeEast.refId = "1"
    stage.refId = "3"
  ...
  def "image is not resolved from bake if cloud provider does not match"() {
    given:
    def bakeEast = new StageExecution(stage.execution, "bake", [ami: bakeAmi, region: 'us-east-1', cloudProvider: 'gce'])
    bakeEast.refId = "1"
    stage.refId = "3"
    stage.requisiteStageRefIds = [ "1" ]
    stage.context.amiName = null
