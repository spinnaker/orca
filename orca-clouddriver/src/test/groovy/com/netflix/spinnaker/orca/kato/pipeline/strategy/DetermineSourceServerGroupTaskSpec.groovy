...
class DetermineSourceServerGroupTaskSpec extends Specification {
  ...
  void 'should include source in context'() {
    given:
    StageExecution stage = new StageExecution(PipelineExecution.newPipeline("orca"), 'deploy', 'deploy', [
      account          : account,
      application      : 'foo',
  ...
  void 'should useSourceCapacity from context if not provided in Source'() {
    given:
    StageExecution stage = new StageExecution(PipelineExecution.newPipeline("orca"), 'deploy', 'deploy', [useSourceCapacity: contextUseSourceCapacity, account: account, application: application, availabilityZones: [(region): []]])
