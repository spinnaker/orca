...
open class PeeringMetrics(
  ...
  open fun recordLag(executionType: PipelineExecution.ExecutionType, block: () -> Unit) {
    registry
  ...
  open fun incrementNumPeered(executionType: PipelineExecution.ExecutionType, state: ExecutionState, count: Int) {
    registry
      .counter(peeringNumPeeredId.tag(executionType, state))
      .increment(count.toLong())
  }

  open fun incrementNumDeleted(executionType: PipelineExecution.ExecutionType, count: Int) {
    registry
      .counter(peeringNumDeletedId.tag(executionType))
      .increment(count.toLong())
  }

  open fun incrementNumErrors(executionType: PipelineExecution.ExecutionType) {
    registry
      .counter(peeringNumErrorsId.tag(executionType))
      .increment()
  }

  open fun incrementNumStagesDeleted(executionType: PipelineExecution.ExecutionType, count: Int) {
    registry
...
internal fun Id.tag(executionType: PipelineExecution.ExecutionType): Id {
  return this
    .withTag("executionType", executionType.toString())
}

internal fun Id.tag(executionType: PipelineExecution.ExecutionType, state: ExecutionState): Id {
  return this
    .withTag("executionType", executionType.toString())
