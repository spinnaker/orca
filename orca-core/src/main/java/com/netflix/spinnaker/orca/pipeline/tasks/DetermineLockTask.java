...
public class DetermineLockTask implements Task {
  ...
  public TaskResult execute(@Nonnull Stage stage) {
    ...
    Optional<StageNavigator.Result> lockStage = stageNavigator.ancestors(stage).stream().filter(r -> r.getStageBuilder() instanceof AcquireLockStage).findFirst();

    if (lockStage.isPresent()) {
      LockContext lockContext = lockStage.get().getStage().mapTo("/lock", LockContext.class);
